...
| プラットフォーム | ダウンロード |
|:---:|:---:|
| **Windows x64** | [**Markdown Viewer (ZIP)**](https://github.com/choya-tomoki/markdown-viewer/releases/latest) |

1. ZIP ファイルをダウンロードして任意の場所に展開
2. フォルダ内の `Markdown Viewer.exe` を実行
3. （任意）`Markdown Viewer.exe` を右クリック →「ショートカットの作成」でデスクトップに配置

> **SmartScreen の警告が出た場合**: 「詳細情報」→「実行」をクリックしてください（初回のみ）。
> 本アプリはオープンソースで安全ですが、無料のコード署名がないためこの警告が表示されます。

### 方法 2: ソースからビルド（ショートカット自動作成）
...
`scripts\install.bat` をダブルクリックするだけで、依存パッケージのインストール → ビルド → デスクトップショートカット作成まで自動で行います。

完了後、デスクトップの **「Markdown Viewer」** からいつでも起動できます。
